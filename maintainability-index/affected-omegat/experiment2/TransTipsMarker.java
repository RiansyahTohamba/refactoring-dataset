...
public class TransTipsMarker implements IMarker {
    ...
    public List<Mark> getMarksForEntry(SourceTextEntry ste, String sourceText, String translationText,
            boolean isActive) {
        if (isGlossaryEntriesNull() || !Core.isMarkGlossaryMatches() ||
            (!isActive || sourceText == null)) {
            return null;
        }else{
            List<Mark> marks = new ArrayList<Mark>();
            for (GlossaryEntry ent : Core.getGlossary().getDisplayedEntries()) {
    ...
    private boolean isGlossaryEntriesNull() {
