...
public class CalcMatchStatistics extends LongProcessThread {
    ...
    private boolean isExistInPreviousFiles(SourceTextEntry ste){
        // exist in other file
        return alreadyProcessedInProject.contains(ste.getSrcText());
    }

    private boolean isExistInFile(SourceTextEntry ste) {
        // exist in this file
        return alreadyProcessedInFile.contains(ste.getSrcText());
    }
    ...
    private void iterateAllSegments(MatchStatCounts result, List<SourceTextEntry> untranslatedEntries, SourceTextEntry ste) {
        ...
        if (Core.isTranslated(ste)) {
            ...
            result.addExact(count);
            treated++;
        } else if (isExistInPreviousFiles(ste)) {
            result.addRepetitionFromOtherFiles(count);
            entryProcessed();
        } else if (isExistInFile(ste)) {
