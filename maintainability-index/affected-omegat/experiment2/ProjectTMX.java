...
public class ProjectTMX {
    ...
    private class Loader implements TMXReader2.LoadCallback {
        ...
        private String getCreator(TMXReader2.ParsedTuv tuvTarget, TMXReader2.ParsedTu tu){
            if (tuvTarget != null){
                return StringUtil.nvl(tuvTarget.creationid, tu.creationid);
            }else{
                return null;
            }
        }

        private String getChanger(TMXReader2.ParsedTuv tuvTarget,TMXReader2.ParsedTu tu){
            if (tuvTarget != null) {
                return StringUtil.nvl(tuvTarget.changeid, tuvTarget.creationid, tu.changeid,
                        tu.creationid);
            }else{
                return null;
            }
        ...
        public boolean onEntry(TMXReader2.ParsedTu tu,
                               ...
                               TMXReader2.ParsedTuv tuvTarget,
                               boolean isParagraphSegtype)
        {
            if (tuvSource == null) {
                ...
                String translation = getTranslation(tuvTarget);

                List<String> sources = new ArrayList<String>();
                List<String> targets = new ArrayList<String>();

                boolean isNeedResegment = sentenceSegmentingEnabled && isParagraphSegtype;

                Core.segmentEntries(isNeedResegment,sourceLang,
                        tuvSource.text, targetLang,
                        translation, sources, targets);

                String creator = getCreator(tuvTarget,tu);
                String changer = getChanger(tuvTarget, tu);
                long created = getCreated(tu, tuvTarget);
                long changed = getChanged(tu, tuvTarget);
                synchronized (this) {
                    for (int idx = 0; idx < sources.size(); idx++) {
                        iterateSegment(idx, tu, creator, created, changer, changed, sources, targets);
                    }
                }
                return true;
            }
        ...
        private String getTranslation(TMXReader2.ParsedTuv tuvTarget) {
            if (tuvTarget != null) {
                return tuvTarget.text;
            }else{
                return null;
            }
        }

        private long getChanged(TMXReader2.ParsedTu tu, TMXReader2.ParsedTuv tuvTarget) {
            if (tuvTarget != null) {
                return StringUtil.nvlLong(tuvTarget.changedate, tuvTarget.creationdate, tu.changedate,tu.creationdate);
            }else{
                return 0;
            }
        }

        private long getCreated(TMXReader2.ParsedTu tu, TMXReader2.ParsedTuv tuvTarget) {
            if (tuvTarget != null) {
                return StringUtil.nvlLong(tuvTarget.creationdate, tu.creationdate);
            }else{
                return 0;
            }
        }

        private void iterateSegment(int idx,TMXReader2.ParsedTu tu, String creator, long created, String changer, long changed, List<String> sources, List<String> targets) {
            String segmentSource = sources.get(idx);
            String segmentTranslation = targets.get(idx);

            PrepareTMXEntry te = new PrepareTMXEntry();
            setTmxEntProperties(tu, creator, created, changer, changed, segmentSource, segmentTranslation, te);

            EntryKey key = new EntryKey(
                    te.getPropValue(PROP_FILE),
                    te.source,
                    te.getPropValue(PROP_ID), te.getPropValue(PROP_PREV),
                    te.getPropValue(PROP_NEXT),
        ...
        private void setTmxEntProperties(TMXReader2.ParsedTu tu, String creator, long created, String changer, long changed, String segmentSource, String segmentTranslation, PrepareTMXEntry te) {
