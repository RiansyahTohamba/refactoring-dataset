    ...
    public static void loadPlugins() {
        ...
        CoreEvents.registerApplicationEventListener(new IApplicationEventListener() {
            public void onApplicationStartup() {
                ...
                boolean isPrefDefault = Preferences.isPreferenceDefault(Preferences.TAAS_LOOKUP, false);
                lookup.setSelected(isPrefDefault);
                Mnemonics.setLocalizedText(lookup, OStrings.getString("TAAS_MENU_LOOKUP"));
                lookup.addActionListener(e -> { checkClientAllowed(lookup); });
                Core.getGlossaryMenu().add(lookup);
                PreferencesControllers.addSupplier(TaaSPreferencesController::new);
                Preferences.addPropertyChangeListener(
                    Preferences.TAAS_LOOKUP,e -> lookup.setSelected((Boolean) e.getNewValue())
                );
                if (client.isAllowed()) {
            ...
            private void checkClientAllowed(JMenuItem lookup) {
                if (client.isAllowed()) {
